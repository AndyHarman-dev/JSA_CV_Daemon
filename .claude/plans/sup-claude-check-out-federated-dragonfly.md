...
**2026-09-03**: *Context* — user reported that after reasoning started working perfectly on `opencode-go`, three other backends (`gemini`, `openrouter`, `mistral`) streamed no tokens at all, and asked whether that was a model-capability limit or a wiring miss. *Investigation* — it was three separate causes, one per backend. (1) **Gemini: a genuine wiring miss.** `GeminiBackend` overrides `_call_api_once` wholesale and carried its own copy of the `use_stream = on_chunk is not None and structured_schema is None` gate that the earlier `_reasoning_only` fix removed from `_openai_compat.py` and `opencode_zen.py`. Because `_structured_schema_for` returns a schema unconditionally, every real pipeline call fell to the buffered `generateContent` endpoint — nothing could ever stream. (2) **Gemini, second gap:** the payload never set `generationConfig.thinkingConfig.includeThoughts`, so the API never marked any part `"thought": true` and `_consume_gemini_sse`'s split was never true even with streaming on. (3) **OpenRouter: wrong field name.** `_consume_sse` read only `delta.reasoning_content`; OpenRouter emits `delta.reasoning` (legacy string) and `delta.reasoning_details` (array), and needs `reasoning: {"enabled": true}` in the request. (4) **Mistral: never opted in.** Reasoning only exists when `reasoning_effort` is set, and it then arrives as a list-shaped `delta.content` carrying `ThinkChunk` objects, never as `reasoning_content`. *Actions* — user chose "all three backends" over the narrower repair-only options. Added a `_reasoning_payload()` hook to `OpenAICompatBackend` (default `{}`, so non-overriding backends keep a byte-identical payload) with per-backend overrides; generalized `_consume_sse` extraction into `_reasoning_delta_text` (first-non-empty across the three field names, so OpenRouter's duplicated `reasoning`/`reasoning_details` isn't double-counted) and `_split_content_delta` (Mistral's list shape, applied on the non-streaming path too); removed Gemini's dead `structured_schema is None` gate and merged `thinkingConfig` into its `generationConfig`. *Decisions/friction* — (a) The Gemini streaming gate was verified safe to flip only after confirming `_consume_gemini_sse` excludes thought text from `content_parts`; otherwise reasoning would have been concatenated into the JSON body and corrupted every structured parse. (b) Both new request fields can be rejected with a 4xx by a model/upstream that doesn't support them, which under the existing three-way classification is an immediate unretried `AgentBackendUnavailable` — i.e. an optional enrichment could have cost a backend its BF-19 slot. Added `_ReasoningRejected`, a verbatim copy of the `_CacheRejected` shape: caught outside `retry_transient`'s budget, flips `self._reasoning` off for the instance's life, retries once clean. `_call_api`'s single `try/except _CacheRejected` was generalized into a bounded two-degrade loop (reasoning → caching → fail). (c) Blanket 4xx classification was chosen over message-string matching, matching the established `_CacheRejected` convention; the cost is at most two extra HTTP attempts on a genuinely-broken config. (d) Gemini layers `_ReasoningRejected` ABOVE its pre-existing `_SchemaRejected` downgrade so a `thinkingConfig` rejection loses the thinking stream rather than structured mode. (e) `Settings.prompt_caching`'s byte-identity parity assertions in `test_openai_compat.py`/`test_openrouter.py` had to be scoped to the caching fields — the reasoning opt-in is a separate switch and would otherwise silently weaken that invariant. (f) Told the user plainly that the shipped catalog defaults for all three backends (`gemini-3.1-flash-lite`, `mistral-small-2603`, `nvidia/nemotron-3-nano-30b-a3b`) are non-reasoning models, so wiring alone may not produce visible output on their existing runs. *Verification* — full backend suite 1771 passed / 2 skipped (`test_dev_tunnel.py::test_daemon_thread_is_started` flaked once mid-session and passes on a clean tree too; confirmed pre-existing and order-dependent, not a regression). 15 new tests added across `test_streaming_openai_compat.py` (all four field conventions, the no-double-count rule, the Mistral `TypeError` guard on both streaming and buffered paths, `reasoning_effort` presence/absence), `test_openrouter.py` (opt-in presence/absence, full reasoning→caching degrade ordering), and `test_gemini_api.py` (structured-mode streaming, `_reasoning_only` filtering, sentinel-mode passthrough, buffered fallback, `thinkingConfig` presence/absence, thinking-shed-before-schema-downgrade). CLAUDE.md gained a "Reasoning / thinking stream (streaming backends)" section documenting the capability/opt-in/field-convention split and the degrade precedence. **Live end-to-end verification not possible in this environment** — the user should re-run against a reasoning-capable model on each backend and confirm the REASONING card fills in.
