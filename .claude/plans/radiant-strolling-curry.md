...
**2026-09-01**: Phase 2 — cost-ordered model ladder data. Added `jsa/agents/model_costs.py` with static USD/1M-output-token cost tables for all model-selection backends and the `cost_for` / `cost_ordered` / `next_model` ordering primitives. Wired the existing OpenRouter live listing fetch to opportunistically cache real per-model output prices via `record_openrouter_pricing`. Added `tests/backend/test_model_costs.py` with 17 tests covering ascending order, stable ties, unknown-cost-last, next-rung semantics, live-pricing override/fallback/replacement, and a bidirectional guard that every `opencode-go` `_PROTOCOL` model has a `STATIC_COSTS` entry. Verified `.venv/bin/pytest tests/backend/test_model_costs.py` (17 passed), `tests/backend/test_model_catalog.py` (26 passed), and the full non-integration suite (1584 passed with 3 known-flaky failures that pass in isolation — same flaky set as the Phase 1 baseline). Two small deviations from the plan text: `STATIC_COSTS` also includes a `"claude-cli"` key aliased to the Anthropic table (defensible because `claude-cli` supports model selection and runs Claude models, though the plan only listed Mistral/Gemini/Anthropic), and the `claude-cli`/`anthropic` dicts originally shared the same backing dict by reference. A post-commit review follow-up changed `claude-cli` to `dict(_ANTHROPIC_COSTS)` with a comment noting the two tables must be kept in sync by hand. The `hy4-preview` $2.501/1M output price was confirmed against the live OpenCode docs page via webfetch. The user-run OpenCode API probe (Verification #6) remains optional.

**2026-09-01**: Phases 3 & 4 — per-job model tracking and the model-first fallback ladder. Phase 3: added `Job.model_name`/`Job.model_hops` columns (two separate `try/except ALTER TABLE` blocks in `jsa/db/engine.py` so a partial prior migration can't skip the second column); `Orchestrator._wrap_factory` rewritten to detect factory shape by parameter name/count (a `model` parameter, or ≥2 positional params) rather than the old no-default-positional-count metric, which could not tell `(name)` apart from `(name, model=None)`; `server.py::make_backend_factory` gained a per-call `model` argument and `make_model_resolver` (backend → UI-selection-else-flat-default, extracted alongside a shared `_flat_default_model` helper so the mapping is defined once); both reset paths (`soft_reset_job`, `nuclear_reset_job`) now clear `model_name`/`model_hops`. Phase 4: `repo.backend_switch_reset` gained `new_model_name`/`increment_model_hops` kwargs (unconditional assignment means every existing backend-advance call site nulls the model fields for free, in the same commit); `Orchestrator._resolve_model_hop` implements the pure eligibility decision (ladder/resolver configured, `SUPPORTS_MODEL_SELECTION`, pinned-fit escape, hop cap of 5, answered-input brake) and `_advance_backend_or_fail` tries a model hop before the backend advance for `AgentTimeout`/`AgentBackendUnavailable` only (`AgentLimitReached` still skips straight to backend advance); a `ModelSwitchedEvent` was added alongside `BackendSwitchedEvent`. One correction made mid-implementation, flagged by the advisor before coding: the plan's Solutions prose said the per-job rung should be the *new highest precedence* in `_model_for`, but that contradicts Phase 4's own pinned-fit escape (a `--fit-model` pin must stay immovable across a ladder hop) — implemented precedence is `model_override` (`--fit-model`) > per-call rung > runtime UI selection > flat default, with `model_override` first, not third as the prose literally said. A second correction from working through the two skeptic-flagged brake bypasses in the advisor's notes: the answered-input brake checks FollowUps **job-wide**, not scoped to the currently-failing stage — a stage-scoped check misses a still-present answered FollowUp after a `cv_adjust` rewind changes which stage is "current" (that reset branch only deletes *unanswered* FollowUps), and could otherwise be fooled by a `revising_*` reset that deletes *all* FollowUps for that stage as part of taking the hop itself. Added `tests/backend/test_model_tracking.py` (41 tests: column round-trip, `_wrap_factory` three-shape detection, both reset paths) and extended `tests/backend/test_make_backend_factory.py` (precedence + `make_model_resolver`) and `tests/backend/test_bf18_limit_detection.py` (`_resolve_model_hop` unit tests plus end-to-end `Orchestrator.run()` ladder tests, each asserting on a freshly re-read DB row per the plan's own test note). Verified: `.venv/bin/pytest -q -m "not integration"` — 1623 passed, 2 skipped (up from the Phase 2 baseline of 1584 passed + the intervening Phase 1/2 test growth), no regressions; a hand-rolled migration probe confirmed `ALTER TABLE jobs ADD COLUMN model_name`/`model_hops` apply cleanly against a pre-existing (old-schema) sqlite DB. `/code-review high --fix` launched as a background review per CLAUDE.md's Implementation Workflow; its findings, if any, will be folded in as a follow-up commit. Phase 5 (observability/docs) is explicitly out of scope for this session and left untouched.

**2026-09-02**: Phase 5 — observability and docs. Backend: `server.py` now exposes `app.state.model_resolver = make_model_resolver(settings)` (the same closure the orchestrator's ladder uses); `routes_jobs.py::_job_to_dict` gained `model_name` (raw, null until first hop) and `effective_model` (`model_name` if set, else `model_resolver(job.backend_name)`) via a new `_model_resolver_for(request)` helper threaded through all 9 `_job_to_dict` call sites. Frontend: `JobDTO` gained `model_name`/`effective_model`, `WSEvent` gained `model_switched` (mirroring `jsa/events/schema.py::ModelSwitchedEvent`, which Phase 3/4 had already wired backend-side but the frontend never consumed); `store.ts`'s `applyEvent` gained a `model_switched` case that refetches (a hop rewinds job state too, e.g. `running` → `pending`, not just the model, so a plain log-and-ignore would leave the row stale) — deliberately no new store field, since only the job row needs this, not Header's dropdown; `JobList.tsx`'s `JobRow` renders `effective_model` as a small muted mono chip next to the status badge, title-tooltipped via a new `jobList.modelTitle` i18n key. CLAUDE.md gained a `### Model ladder (tried before the backend advance)` subsection under "Backend fallback chain (BF-19)" covering all the mechanics the plan's Solutions listed. Tests: 4 new backend tests (`TestModelDisplay` in `test_api.py`) and 3 new frontend tests (`store.test.ts`'s `model_switched` case, two `JobList.test.tsx` cases for the chip present/absent) — all additive, no existing test edited. Verified: `.venv/bin/pytest -q -m "not integration"` — 1627 passed, 2 skipped (up from Phase 3/4's 1623+2); `npm test -- --run` — 283 passed (up from 273); `tsc --noEmit` clean; `npm run build` succeeded (both before and after the i18n catch-up below). Ran `.venv/bin/python -m jsa.i18n.translate --backend cli` (no `ANTHROPIC_API_KEY` in this shell, so used the CLI backend per its own fallback) — this caught up not just the new `jobList.modelTitle` key but a pre-existing backlog of 5 untranslated `header.model*` keys from the separate multi-backend-model-select plan's Phase 6, which had apparently shipped without a translate-ui.sh pass; `--check` now passes clean across all 20 locales. Manual verification (plan's Verification item 7): started `jsa` against a scratch DB/CSV, hand-set a job's `backend_name`/`model_name`/`state` directly via sqlite to simulate a post-hop job, confirmed `GET /api/jobs` returned the expected `effective_model`, and took a Playwright screenshot confirming the chip renders correctly in the dark theme without colliding with the status badge — sent to the user. Verification items 1-3, 5-6 (full suite already covered above; live repro and the OpenCode cost probe) remain user-actions as before, unattempted this session.
