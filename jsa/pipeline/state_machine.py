...
def transition(job, new_state: JobState, new_stage: Stage | None = None) -> None:
    ...
    # running → cv_done: valid when current stage is cv_adjust (normal completion) OR
    # cover_letter (backend-switch restart: rewind to cv_done so the new backend
    # re-runs cover_letter from a clean checkpoint).
    ...
    if job.state == JobState.running and new_state == JobState.cv_done:
        if job.current_stage not in (Stage.cv_adjust, Stage.cover_letter):
            raise InvalidTransition(
                f"Can only reach cv_done from running(cv_adjust/cover_letter), got running({job.current_stage})"
            )
